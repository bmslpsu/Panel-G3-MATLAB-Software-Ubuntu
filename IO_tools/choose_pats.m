...
%cd(pattern_path)
cd('/home/jean-michel/Documents/MATLAB/MatlabCodes/Patterns')
...
%         dos(['del ' SD_drive ':\*.pat']);
            system(['rm ' SD_drive '/*.pat']);
    end

%      result1 = dos(['copy /Y "' temp_path '\*.pat" ' SD_drive ':\']);  %% SS
     result1 = system(['cp ' temp_path '/*.pat ' SD_drive]);  %% SS
    ...
    if result1
        ...
        if length(dir([SD_drive, '/*.pat']))
            system(['del ' SD_drive '/*.pat']);
        end
        
        result2 = system(['cp ' temp_path '/*.pat ' ' /media/jean-michel/' SD_drive]);  %% SS
